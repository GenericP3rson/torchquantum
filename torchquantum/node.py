...
class QuantumNode(tq.QuantumModule):
    ...
    def __init__(self, arch, act_norm, node_id):
        ...
        if self.act_norm == "batch_norm" or self.act_norm == "batch_norm_no_last":
    ...
    def forward(self, x, use_qiskit=False, is_last_node=False, parallel=True):
        ...
        if isinstance(self.noise_model_tq, tq.NoiseModelTQActivation):
            x = self.noise_model_tq.add_noise(x, self.node_id, is_after_norm=False)
        ...
        if self.act_norm == "layer_norm":
            ...
            if self.pre_specified_mean_std is None:
                ...
                x = (
                    x
                    - torch.tensor(
                        self.pre_specified_mean_std["mean"], device=x.device
                    ).unsqueeze(0)
                ) / torch.tensor(
        ...
        if isinstance(self.noise_model_tq, tq.NoiseModelTQActivation):
            x = self.noise_model_tq.add_noise(x, self.node_id, is_after_norm=True)
        ...
        if use_qiskit:
            with torch.no_grad():
                ...
                (
                    x,
                    time_spent_list,
                ) = self.qiskit_processor.process_parameterized_and_shift(
                    self.q_device,
                    self.encoder,
                    self.q_layer,
                    self.measure,
                ...
                results = x.reshape(
                    1 + 2 * np.sum(self.shift_this_step), bsz, self.arch["n_wires"]
                )
